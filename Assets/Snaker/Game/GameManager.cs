...
namespace Snaker.Game
{
    public class GameManager:ServiceModule<GameManager>
    {
        ...
        /// 帧事件
        /// </summary>
        /// <param name="vkey">帧事件的key定义，例如是创建玩家事件等</param>
        /// <param name="arg">携带的参数</param>
        /// <param name="playerId">这个帧事件属于某个玩家的id</param>
        ...
        private void HandleOtherVKey(int vkey, float arg, uint playerId)
        {
            ...
            hasHandled = hasHandled || DoVKey_CreatePlayer(vkey, arg, playerId);//是否是创建玩家，是且创建成功，不会走到下面
